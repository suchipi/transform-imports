...
class NormalRequire extends ImportDefinition {
  constructor(path) {
    this.path = path;
  }
  ...
  set path(newPath) {
    const isValidPath = newPath.isVariableDeclarator(); // const `foo = require("bar")`;

    if (!isValidPath) {
      throw new Error(
        "Attempted to set the path of an NormalRequire to an invalid path type: " +
          newPath.type
      );
    }
